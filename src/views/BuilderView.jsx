...
const QUESTION_TYPES = [
  { type: "short_text", label: "Short answer",   description: "Single line",   icon: Type,        iconBg: "bg-teal-50",   iconColor: "text-teal-600",   badge: "bg-teal-50 text-teal-700" },
  { type: "text",       label: "Long text",       description: "Paragraph",     icon: AlignLeft,   iconBg: "bg-teal-50",   iconColor: "text-teal-600",   badge: "bg-teal-50 text-teal-700" },
  { type: "choice",     label: "Multiple choice", description: "Single select", icon: Circle,      iconBg: "bg-violet-50", iconColor: "text-violet-600", badge: "bg-violet-50 text-violet-700" },
  { type: "checkbox",   label: "Checkboxes",      description: "Multi select",  icon: CheckSquare, iconBg: "bg-violet-50", iconColor: "text-violet-600", badge: "bg-violet-50 text-violet-700" },
  { type: "rating",     label: "Star rating",     description: "1 – 5 stars",   icon: Star,        iconBg: "bg-amber-50",  iconColor: "text-amber-500",  badge: "bg-amber-50 text-amber-700" },
  { type: "nps",        label: "NPS score",       description: "0 – 10 scale",  icon: BarChart2,   iconBg: "bg-indigo-50", iconColor: "text-indigo-600", badge: "bg-indigo-50 text-indigo-700" },
  { type: "yesno",      label: "Yes / No",        description: "Binary choice", icon: ToggleLeft,  iconBg: "bg-rose-50",   iconColor: "text-rose-500",   badge: "bg-rose-50 text-rose-700" },
];
...
function getAISuggestions(input) {
  ...
  for (const t of AI_TEMPLATES) {
    if (t.keywords.some((k) => lower.includes(k))) {
      return t.questions.map((q, i) => ({ ...q, id: "ai_" + i, options: q.options ?? [] }));
    }
  }
...
function AnswerPreview({ q, onAddOption, onRemoveOption, onUpdateOption }) {
  switch (q.type) {
    case "short_text":
      return <div className="mt-4 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-2.5"><p className="text-sm text-slate-400">Short text — single line</p></div>;
    case "text":
      return <div className="mt-4 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-4"><p className="text-sm text-slate-400">Long text — paragraph answer</p></div>;
    case "choice":
    case "checkbox":
      return (
    ...
    case "rating":
      return (
        <div className="mt-4">
          ...
          <p className="mt-2 text-xs text-slate-400">Respondents pick 1 – 5 stars.</p>
        </div>
      );
    case "nps":
      return (
        <div className="mt-4">
          ...
          <div className="mt-1.5 flex justify-between text-[11px] text-slate-400"><span>Not at all likely</span><span>Extremely likely</span></div>
        </div>
      );
    case "yesno":
      return (
        <div className="mt-4 flex gap-3">
          <div className="flex flex-1 items-center justify-center rounded-xl border-2 border-teal-200 bg-teal-50 py-3 text-sm font-semibold text-teal-700">Yes</div>
          <div className="flex flex-1 items-center justify-center rounded-xl border-2 border-rose-200 bg-rose-50 py-3 text-sm font-semibold text-rose-600">No</div>
        </div>
      );
    default:
      return null;
...
function QuestionCard({ q, idx, total, isSelected, onSelect, onMoveUp, onMoveDown, onUpdate, onRemove, onAddOption, onRemoveOption, onUpdateOption }) {
  ...
  return (
    <div onClick={onSelect} className={`group relative cursor-pointer rounded-2xl border bg-white transition-all ${isSelected ? "border-indigo-300 shadow-md shadow-indigo-100/50" : "border-slate-200 hover:border-slate-300 hover:shadow-sm"}`}>
      ...
      <div className="p-5">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5">
            ...
            <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${meta.badge}`}>{meta.label}</span>
          </div>
          <div className={`flex items-center gap-0.5 transition-opacity ${isSelected ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
            <button onClick={(e) => { e.stopPropagation(); onMoveUp(); }} disabled={idx === 0} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-25 transition-colors" title="Move up"><ChevronUp size={14} /></button>
            <button onClick={(e) => { e.stopPropagation(); onMoveDown(); }} disabled={idx === total - 1} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-25 transition-colors" title="Move down"><ChevronDown size={14} /></button>
        ...
        <input value={q.label} onChange={(e) => { e.stopPropagation(); onUpdate(q.id, { label: e.target.value }); }} onClick={(e) => e.stopPropagation()} placeholder="Type your question here…" className="mt-4 w-full bg-transparent text-[15px] font-medium text-slate-800 placeholder-slate-300 outline-none border-b border-transparent focus:border-indigo-200 pb-1 transition-colors" />
        ...
        {isSelected && (
          <div className="mt-4 flex items-center gap-4 border-t border-slate-100 pt-3">
            <label onClick={(e) => e.stopPropagation()} className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-xs text-slate-500 select-none">Required</span>
            </label>
            <input
              value={q.helpText ?? ""}
              ...
              className="flex-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-600 placeholder-slate-300 outline-none focus:border-indigo-300"
            />
          </div>
        )}
      </div>
...
function LeftPanel({ onAdd }) {
  return (
    <div className="hidden lg:flex w-48 shrink-0 flex-col border-r border-slate-200 bg-white overflow-y-auto">
      <div className="px-3 pt-4 pb-2">
        <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400">Question types</p>
      </div>
      <div className="grid grid-cols-2 gap-1.5 px-3 pb-4">
        {QUESTION_TYPES.map((qt) => (
          <button
            ...
            className="group flex flex-col items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-2 py-3 text-center transition-all hover:border-indigo-200 hover:bg-indigo-50"
          >
...
function AiPanel({ onAddQuestion }) {
  ...
  return (
    <div className="hidden xl:flex w-[26rem] shrink-0 flex-col border-l border-slate-200 bg-white overflow-hidden">
      {/* Header */}
      <div className="shrink-0 border-b border-slate-100 px-5 py-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-800">AI Question Generator</p>
            <p className="text-[11px] text-slate-400">Describe your topic, get questions instantly</p>
          </div>
        </div>
      </div>
      ...
      <div className="shrink-0 border-b border-slate-100 p-4">
        <textarea
          ...
          placeholder={"e.g. A car review survey for Tesla Model 3 owners after their first month of driving…"}
          rows={5}
          className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 text-sm text-slate-700 placeholder-slate-400 outline-none transition-all focus:border-indigo-300 focus:bg-white focus:ring-2 focus:ring-indigo-100"
        />
        <button
          onClick={generate}
      ...
      <div className="flex-1 overflow-y-auto">
        {loading && (
          <div className="flex flex-col items-center justify-center py-14 gap-3">
            ...
            <p className="text-sm text-slate-500">Crafting questions…</p>
            <p className="text-xs text-slate-400">Analysing your topic</p>
          </div>
        )}
        ...
        {!loading && suggestions.length > 0 && (
          <div className="p-4 space-y-2.5">
            ...
            {suggestions.map((s) => {
              ...
              return (
                <div key={s.id} className={`rounded-xl border p-3.5 transition-all ${isUsed ? "border-teal-200 bg-teal-50" : "border-slate-200 bg-white hover:border-indigo-200"}`}>
                  <div className="flex items-start gap-2.5">
                    <div className={`mt-0.5 shrink-0 rounded-lg p-1.5 ${meta.iconBg}`}>
                      <meta.icon size={12} className={meta.iconColor} />
                    </div>
                    <p className="flex-1 text-xs font-medium leading-relaxed text-slate-700">{s.label}</p>
                  </div>
                  <button
                  ...
                  >
                    {isUsed ? "✓ Added to survey" : "+ Add to survey"}
                  </button>
                </div>
              );
            })}
        ...
        {!loading && suggestions.length === 0 && (
          <div className="flex flex-col items-center justify-center py-14 px-6 text-center">
            ...
            <p className="mt-1.5 text-xs leading-relaxed text-slate-400">
              Describe your survey topic above — e.g. "car review", "restaurant feedback", "employee pulse" — and we'll suggest tailored questions.
            </p>
          </div>
        )}
      </div>
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      ...
      <div className="flex flex-1 min-w-0 flex-col overflow-hidden">
        ...
        {/* ── Builder header ────────────────────────────────────────────── */}
        <div className="shrink-0 flex h-14 items-center gap-2 border-b border-slate-200 bg-white px-4 sm:px-5">
          <button className="shrink-0 rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 sm:hidden" onClick={() => setMobileOpen(true)}>
            <Menu size={18} />
          </button>

          <button onClick={() => go("dashboard")} className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-800">
            <ArrowLeft size={15} />
          ...
          <div className="hidden items-center gap-1.5 sm:flex">
            ...
            <button onClick={() => go("dashboard")} className="max-w-[120px] truncate text-sm text-slate-400 transition-colors hover:text-indigo-600">{activeWorkspace.name}</button>
            <ChevronRight size={14} className="text-slate-300" />
            <span className="text-sm font-medium text-slate-700">{isEditing ? "Edit Survey" : "New Survey"}</span>
          </div>
          ...
          <span className="hidden text-xs text-slate-400 sm:block">{questions.length} question{questions.length !== 1 ? "s" : ""}</span>
          <div className="h-4 w-px bg-slate-200" />

          <button onClick={() => go("dashboard")} className="rounded-xl border border-slate-200 px-3.5 py-1.5 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50">
            Cancel
          </button>
          <button onClick={deploy} disabled={surveyAtLimit} className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-4 py-1.5 text-sm font-semibold text-white transition-colors hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50">
            {surveyAtLimit ? <><Lock size={13} /> Upgrade</> : isEditing ? <><CheckCircle2 size={13} /> Save changes</> : <><Rocket size={13} /> Publish</>}
          </button>
        </div>
            ...
            <div className="mx-auto max-w-xl space-y-4 px-4 py-8 sm:px-6">
              ...
              {surveyAtLimit && (
                <div className="flex items-start justify-between gap-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3.5">
                  <div>
                    <p className="flex items-center gap-1.5 text-sm font-semibold text-amber-800"><Lock size={14} /> Survey limit reached</p>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white px-6 py-6">
                <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Untitled Survey" className="sp-display w-full bg-transparent text-2xl font-bold text-slate-900 placeholder-slate-300 outline-none" />
                <textarea value={description} onChange={(e) => setDesc(e.target.value)} rows={2} placeholder="Add a short description (optional)…" className="mt-3 w-full resize-none bg-transparent text-sm text-slate-500 placeholder-slate-300 outline-none" />
              </div>
              ...
              <div className="lg:hidden rounded-2xl border border-dashed border-slate-200 bg-white p-4">
                <p className="mb-3 text-[10px] font-semibold uppercase tracking-widest text-slate-400">Add a question</p>
                <div className="grid grid-cols-4 gap-1.5">
                  {QUESTION_TYPES.map((qt) => (
                    <button key={qt.type} onClick={() => addQuestion(qt.type)} className="group flex flex-col items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 py-3 px-2 text-center transition-all hover:border-indigo-200 hover:bg-indigo-50">
                      ...
                      <p className="text-[10px] font-semibold leading-tight text-slate-600 group-hover:text-indigo-600">{qt.label}</p>
                    </button>
                  ))}
                </div>
              </div>
      ...
      {deployed && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-6" onClick={() => setDeployed(null)}>
          <div className="w-full max-w-sm rounded-2xl bg-white p-7 text-center sp-rise" onClick={(e) => e.stopPropagation()}>
            <div className="mx-auto mb-4 inline-flex h-12 w-12 items-center justify-center rounded-full bg-teal-50">
              <CheckCircle2 size={24} className="text-teal-500" />
            </div>
            <h3 className="sp-display text-lg font-bold text-slate-900">{isEditing ? "Changes saved" : "Survey is live!"}</h3>
            <p className="mt-1.5 text-sm text-slate-500">
              {isEditing
                ? <><span className="font-semibold text-slate-700">{deployed.title}</span> has been updated.</>
                : <><span className="font-semibold text-slate-700">{deployed.title}</span> is now collecting responses.</>}
            </p>

            {!isEditing && (
              <>
                <div className="mx-auto mt-5 flex h-36 w-36 items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50">
                  <QrCode size={68} className="text-slate-600" strokeWidth={1.2} />
                </div>
                <div className="mt-4 flex items-center justify-between rounded-xl bg-slate-50 px-3.5 py-2.5">
                  <span className="sp-mono truncate text-sm text-slate-600">{deployed.endpoint}</span>
            ...
            <div className="mt-5 flex gap-2.5">
              {!isEditing && (
                <button className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-slate-200 py-2.5 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50">
